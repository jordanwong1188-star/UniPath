...
export function ApplicationHub({ mode, initialApplicationId, showChooser = true }: { mode: "scholarships" | "applications"; initialApplicationId?: string; showChooser?: boolean }) {
  ...
  return <main className="min-h-screen bg-[#f4f1ea] text-[#172126]">
    ...
    <section className="mx-auto max-w-7xl px-6 py-10 lg:px-10">
      ...
      </div> : !ready ? <div className="mt-8 rounded-3xl bg-white p-12 text-center text-gray-500">Opening application tools…</div> : <div className="mt-8">
        ...
        <section className="mt-6 rounded-3xl border border-[#692f46]/10 bg-white p-6 shadow-sm sm:p-8">
          ...
          <div className="mt-7 grid gap-5 lg:grid-cols-[1fr_310px]">
            <div className="rounded-2xl bg-[#f7f4ee] p-5 sm:p-6">
              <div className="flex flex-wrap items-center justify-between gap-3"><span className="rounded-full bg-[#692f46] px-3 py-1 text-xs font-semibold uppercase tracking-[0.1em] text-white">{practiceMode === "written" ? "Written" : "Video"} practice · question {questionIndex + 1}</span><div className="flex items-center gap-4">{isPremium ? <button type="button" disabled={!draft.trim()} onClick={saveCurrentDraft} className="inline-flex cursor-pointer items-center gap-2 text-sm font-semibold text-[#692f46] disabled:opacity-40"><Save className="h-4 w-4" /> {savedAt || "Save draft"}</button> : <Link href="/pricing" className="inline-flex items-center gap-2 text-sm font-semibold text-[#692f46]"><Save className="h-4 w-4" />Save with Pro</Link>}<button type="button" onClick={() => { setQuestionIndex(current => (current + 1) % questions.length); setDraft(""); setPrompt(""); setAttemptId(undefined); setSavedAt(""); resetTimer(); }} className="inline-flex cursor-pointer items-center gap-2 text-sm font-semibold text-[#692f46]"><RefreshCw className="h-4 w-4" /> New question</button></div></div>
          ...
          <div className="mt-5 flex flex-col gap-4 rounded-2xl border border-[#692f46]/10 bg-[#fffaf5] p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">Finished this attempt?</p><p className="mt-1 text-sm text-gray-500">{isPremium ? "Get transparent rubric feedback before trying the next question." : "Written and video practice are free to use. AI feedback and saved attempts require Pro."}</p></div>{isPremium ? <button type="button" disabled={!draft.trim()} onClick={requestFeedback} className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-xl bg-[#692f46] px-5 py-3 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40">{feedbackLoading ? <LoaderCircle className="h-4 w-4 animate-spin" /> : null}{feedbackLoading ? "Reviewing response…" : "Finish practice & get feedback"}</button> : <Link href="/pricing" className="inline-flex items-center justify-center rounded-xl bg-[#692f46] px-5 py-3 text-sm font-semibold text-white">Unlock feedback and saving</Link>}</div>
