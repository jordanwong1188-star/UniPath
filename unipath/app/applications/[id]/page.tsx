...
export default function ProgramApplicationPage() {
  ...
  const selected = applicationProfiles.find(item => item.id === id);
  ...
  return <main className="min-h-screen bg-[#132c29]">
    <nav className="sticky top-0 z-[60] border-b border-white/10 bg-[#132c29]/95 px-5 py-3 text-[#f2ede2]" aria-label="Practice format">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3">
        <div className="min-w-0 flex-1"><p className="text-[9px] font-bold uppercase tracking-[.18em] text-[#e0a17f]">Practice file</p><p className="mt-1 truncate text-sm font-semibold">{selected.program}</p></div>
        <div className="flex shrink-0 gap-px border border-white/12 bg-white/10 text-xs font-bold">
    ...
    <section id="written-practice">
      <ApplicationHub key={selected.id} mode="applications" initialApplicationId={selected.id} showChooser={false} />
    </section>
    {hasVideo ? <section id="interview-practice" className="scroll-mt-24"><VideoInterviewSimulator key={selected.id} profile={selected} /></section> : null}
  </main>;
}
