...
const links = [["Universities", "/universities"], ["Programs", "/programs"], ["Admissions Fit", "/admissions-fit"], ["Transfers", "/transfers"], ["Scholarships", "/scholarships"], ["Applications", "/applications"]];
...
export default function SiteHeader({ dark = false }: { dark?: boolean }) {
  ...
  return <header className={`relative z-50 border-b backdrop-blur-xl ${dark ? "border-white/10 bg-[#0f1823]/90 text-white" : "border-black/10 bg-[#d8dde0]/90 text-[#182431]"}`}>
    <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-6 lg:px-10">
      ...
      <nav className="hidden items-center gap-1 rounded-full border border-current/10 bg-current/[0.035] p-1 text-sm font-semibold lg:flex">{links.map(([label, href]) => <Link key={href} href={href} className="rounded-full px-3 py-2 opacity-65 transition hover:bg-current/[0.06] hover:opacity-100">{label}</Link>)}</nav>
