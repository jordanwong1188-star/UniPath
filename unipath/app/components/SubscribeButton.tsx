...
export default function SubscribeButton({ plan, label, featured = false }: { plan: "pro" | "max"; label: string; featured?: boolean }) {
  ...
  return <div className="mt-8">
    <button type="button" onClick={beginCheckout} disabled={loading} className={`flex w-full items-center justify-between px-4 py-3 text-sm font-semibold disabled:cursor-wait disabled:opacity-60 ${featured ? "bg-[#d4865f] text-[#132c29]" : "border border-white/18 text-white"}`}>
      {loading ? "Opening secure checkout…" : label}<ArrowRight className="h-4 w-4" />
    </button>
